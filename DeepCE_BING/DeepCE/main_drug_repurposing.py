        ...
        for line in f:
            ...
            if g in l1000_gene and 'NA' not in line:
                ...
                if sig_type == 'NCBI':
                    ...
                    patient_sig.append(float(line[3])) # standard error of logFC from the DeSeq2 analysis
...
print(f'L1000 gene file: {l1000_gene_file}')
l1000_gene = get_l1000_gene(l1000_gene_file)
print(f'Number of L1000 genes: {len(l1000_gene)}')
drug_list = get_drug_list(predicted_drug_file)
print(f'Number of predicted drugs: {len(drug_list)}')

print('***** Getting Disease Signature *****')
print(f'Patient file: {patient_file}')
idx, patient_sig = get_disease_signature(patient_file, l1000_gene, sig_source)
# patient_sig -> standard error of log2FC estimates from DeSeq2
print(f'Number of genes in patient signature: {len(idx)}')

output_total_correlation = []
for f1, f2 in zip(predicted_drug_sig_file_list, hq_drug_sig_file_list):
    print(f1, f2)
    drug_sig = read_drug_signature(f1, idx) # predicted drug gene expression profiles
    hq_drug_sig, hq_drug_list = read_hq_drug_signature(f2, idx) # gene expression profiles from the high-quality part of the L1000 dataset
    new_drug_list = drug_list + hq_drug_list
    drug_sig = np.concatenate((drug_sig, hq_drug_sig), axis=0)
    correlation = compute_correlation(drug_sig, patient_sig)
    print(f'Drug Sig: {drug_sig.shape}, HQ Drug Sig: {hq_drug_sig.shape}, Correlation: {correlation.shape}')
    output_correlation = get_candidate_drug_id(correlation, new_drug_list, num_candidate=top) # get list of 'top' candidate drugs
